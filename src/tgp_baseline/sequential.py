...
class SequentialMarketEnvironment:
    ...
    def get_current_market_state(
        ...
        # Get competitor prices (always use real historical data)
        ...
        # Handle agent prices
        if agent_prices is None:
            agent_prices = {}

        # Store agent prices during interaction phase
        if not is_observation_phase and agent_prices:
...
Use your market analysis to compete effectively and optimize profits."""
    ...
    def _create_observation_prompt(
        ...
        observation_summary = (
            "\n".join(
                [
                    f"Period {obs['period']}: {obs['observations'][:100]}..."
...
"""
    ...
    def _create_interaction_prompt(
        ...
        # Format current prices (mix of competitors and agents)
        active_prices = market_state.competitor_prices.copy()
        active_prices.update(market_state.agent_prices)

        if active_prices:
            price_info = "\n".join(
                [f"  • {brand}: {price:.1f}p" for brand, price in active_prices.items()]
            )
        else:
            price_info = "  • No pricing data available"
...
Consider competitor behavior patterns, market dynamics, and profit optimization.
...
3. Market Response: How competitors might react to your pricing
...
What price will maximize your profitability?
"""
        return prompt
...
def run_sequential_experiment(
    ...
    print(f"   Observation periods: {market_env.transition_period}")
    print(f"   Interaction periods: {period - market_env.transition_period}")
...
if __name__ == "__main__":
    ...
    market_env = SequentialMarketEnvironment(
        observation_start="2009-04-01",  # When coordination began
        interaction_start="2010-03-01",  # When margins doubled
        end_date="2011-04-01",  # "2012-08-31",  # End of coordination period
    )
