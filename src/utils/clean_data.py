...
def preprocess_tgp_data(
    data_path: str = "../../data/raw/tgp",
    tgpmin_path: str = "../../data/tgpmin.csv",
    output_path: str = "../../data/preprocessed/tgp.parquet",
    log_file: str = "../../data/logs/tgp.log",
) -> None:
    ...
    logger.info("Original paper: 6 terminal gates")
    ...
    df_tgpmin = df_tgpmin.with_columns(
    ...
    df_combined = df.join(
        df_tgpmin, left_on="publish_date", right_on="date", how="inner"
    )
    ...
    logger.info(
        "TGP MIN Combined Data Equal Counts\n", df_combined["equal"].value_counts()
    )
...
def preprocess_retail_data(
    data_path: str = "../../data/raw/retail",
    output_path: str = "../../data/preprocessed/data_avg_brand.parquet",
    log_file: str = "../../data/logs/retail.log",
) -> None:
    ...
    df = (
        df.group_by(["publish_date", "brand_description"])
        .agg(pl.col("product_price").mean().round(2).alias("avg_price"))
