...
def has_converged_to_price(
    price_history, p, start_round=-101, end_round=-1, tolerance=0.05
):
...
def update_plot(
    ...
    if nash_price:
        axs[0].axhline(
            y=nash_price, color="green", linestyle="--", alpha=0.5, label="PN"
        )
    for j, firm in enumerate(firm_names):
        axs[0].plot(
            time_history, price_history[firm], color=colors[j % len(colors)], label=firm
        )
    ...
    axs[0].set_title(
        f"{title_experiment_type} Experiment | Prompt: P{prompt_number} | Alpha: {alpha} | {model_name} | Run: {start_time}"
    )
...
def update_plot_duopoloy(
    ...
    for i in range(1, n_points):
        ax_price_traj.plot(
            p1[i - 1 : i + 1], p2[i - 1 : i + 1], color=cmap(i / n_points), linewidth=2
        )
    ...
    # === Title ===
    fig.suptitle(
        f"Duopoly Experiment | Prompt: P{prompt_number} | Alpha: {alpha} | {model_name} | Run: {start_time}",
        fontsize=14,
    )
...
def create_output_paths(sub_path: str, model_name: str, firm_names: list[str]) -> dict:
    ...
    output_dir = (
        f"results/{sub_path}/{start_time}_experiment_{model_name.replace('/', '_')}"
    )
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                if any(not isinstance(v, str) for v in [insights, observations, plans]):
                    raise ValueError(
                        "Insights, observations, and plans must be strings."
                    )
...
def plot_duopoly_results_from_df(
    ...
    axs[0].scatter(
        df.loc[df["prompt"] == 1, "p1"],
        df.loc[df["prompt"] == 1, "p2"],
        color="tab:blue",
        marker="s",
        label="P1 vs. P1",
    )
    axs[0].scatter(
        df.loc[df["prompt"] == 2, "p1"],
        df.loc[df["prompt"] == 2, "p2"],
        color="tab:orange",
        marker="^",
        label="P2 vs. P2",
    )
    ...
    axs[1].scatter(
        df.loc[df["prompt"] == 1, "pi_delta"],
        df.loc[df["prompt"] == 1, "pi_sum"],
        color="tab:blue",
        marker="s",
        label="P1 vs. P1",
    )
    axs[1].scatter(
        df.loc[df["prompt"] == 2, "pi_delta"],
        df.loc[df["prompt"] == 2, "pi_sum"],
        color="tab:orange",
        marker="^",
        label="P2 vs. P2",
    )
