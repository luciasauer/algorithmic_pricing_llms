...
def update_plot(
    ...
    axs[0].plot(
        time_history, price_history, marker="o", color="blue", label="Agent Price"
    )
    ...
    axs[1].plot(
        time_history, quantity_history, marker="s", color="green", label="Quantity"
    )
    ...
    axs[0].set_title(
        f"Monopoly Experiment {model_name} ({start_time}) | Converged: {converged}"
    )
